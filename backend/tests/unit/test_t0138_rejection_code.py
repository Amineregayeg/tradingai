...
async def test_the_vocabulary_is_CLOSED_and_every_code_is_distinct():
    ...
    # `0016`: the kill switch was armed when the order reached the adapter's send, so nothing was sent), 22 once
    # `SUBMISSION_NOT_FOUND_AFTER_RESTART` did (`T-0144` S4, migration `0017`: a pre-send record left by a crash
    # whose client order id two venue lookups did not find).
    ...
    assert len(REJECTION_CODES) == len(set(REJECTION_CODES)) == 22
