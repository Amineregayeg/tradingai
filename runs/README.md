...
|---|---|
| runs | **24** |
| decision records | **1678** |
| trade rows | **289** |
| telemetry records | 7894 |
| candles | 8937 |

**DO NOT read a single P&L figure off this table.** Of the 289 trade rows,
...
| 24 | [`fe837dd1`](RUN-24__2026-08-24__fe837dd1.md) | `2026-08-24T15:45:20` | — | 485 | 477 | 8 | 9 | 6 | 23.79 |
...
**CORRECTION, 2026-08-30.** An earlier version said *"Every other run has real per-position ids
(5 distinct across runs 02-24), so the column means what it says from Run 02 onward."* **That was
false.** The "5 distinct" count is real but it is 4 genuine ids belonging to two runs plus the
literal `'paper'` shared by twelve — a measured number attached to a conclusion never checked.
Trade counts, P&L and decision figures are unaffected; only the grouping is wrong. Found while
answering the owner's audit ([AUDIT_RESPONSE_2026-08-30.md](AUDIT_RESPONSE_2026-08-30.md), Q7.2b).
...
* `data/trades.csv` — all 289 trade rows across every run, all columns.
* `data/CHECKSUMS.txt` — SHA-256 of every data file, so a later copy can be checked
  against this one.
