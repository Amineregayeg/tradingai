...
**WHY THIS FIX GETS SKIPPED, which matters more than the fix — Review, after the shape recurred three
more times the same day, twice inside the corrections for earlier instances:**

    "is the answer right"                         has an OBVIOUS method
    "could this have produced a different answer"  has NONE

> **The two questions look identical in prose and only one of them has a procedure. So the second gets
> skipped BY DEFAULT rather than by decision** — which is why naming it as a rule is not enough on its
> own, and why every instance so far was caught by **another seat checking the premise** rather than by
> its author re-reading.

**Six instances between two seats in one day** — three mutation attempts, a string-compare artefact, a
fail-open nearly reported backwards, a proposed fixture that was vacuous, a corpus glob that read empty
as absence, and a control pair whose arms were one commit. **Not carelessness in either seat: the
asymmetry above, plus the fact that a clean answer is never investigated.**

### AND THE MUST-MISS ARM HAS A DECAY MODE: documenting a control invalidates it — Execute, `B136`

**`zzz_absent` was this loop's house must-miss token. It no longer works against this file.**

    grep -c 'zzz_absent' KNOWN_ISSUES.md   ->  4        <- it is IN the corpus it was meant to be absent from
    first introduced by                        66b9f03  <- the Manager's own commit, documenting a control pair

> **A control token written into the searched corpus stops being a control.** Every entry that records
> *"control must-miss: `zzz_absent` → 0"* **adds an occurrence**, and the arm dies **silently** — a
> must-miss returning `1` looks like a finding rather than a broken instrument. **Execute's must-miss
> returned a HIT and that is how it surfaced.**

**THE FIX, Execute's:** **use a task id plus a nonce, never a shared house token** — `zzz_T0030_absent`,
`zzz_B141_absent`. **A shared token accumulates references; a scoped one cannot, because nothing else
ever writes it.**

**Nothing already published is invalidated by this** — the affected arms were run against `app/`,
`tests/` and the filesystem, where the token genuinely is absent. **That is luck rather than design:
the one seat that pointed a must-miss at this file got the hit.**
...
### B138 — `TIGHTER_THAN_NECESSARY` had 2680 chances to fire across the swept range and fired ZERO times; `RR_ABOVE_ACCEPTABLE_BAND` fires on 84-100%
...
**Review's finding, and it is the one that turns this result from uninterpretable into a finding.** As
first filed, this entry explained the 0% by quoting *"accepted candidates per setup runs 1.00-1.34"*.

> **That is a MEAN offered as evidence about a TAIL.** `GATE-030` fires only when something tighter was
> chosen over something wider THAT ALSO CLEARED 2R, so it needs **two accepted candidates on the same
> selection** before it can fire at all. A mean of 1.00-1.34 is equally consistent with *"no selection
> ever had two survivors, so 0% is vacuous"* and *"many did and it never fired, which is a finding"* —
> **and both print `0%` and pass every pin.**

**Measured: 2680 selections across the sweep DID have two or more accepted candidates, 31 to 157 per
row.** So the flag was reachable throughout and never fired. **It is a finding, not a vacuum** — and
without the denominator nobody could have told which.

**A stricter verdict `UNREACHABLE` now fires when total opportunities are zero. It can only turn
REPORTABLE into UNMEASURED, never the reverse, and the pinned 50% is untouched.**

**AND THE FIX SHIPPED A BUG THAT ITS OWN FIGURE EXPOSED.** The first correction applied `GATE-030`'s
denominator to ALL THREE flags. `GATE-029` and `GATE-031` read `selected_stop.rr` alone and fire on a
single accepted candidate, so their numerator counted firings the denominator excluded:

    RR_ABOVE_ACCEPTABLE_BAND    fires on 284.1%-1425.8% of ...      <- an impossible rate

**An impossible rate is the cheapest failure available and it was caught by READING the figure, not by
anything mechanical.** `MIN_ACCEPTED_TO_FIRE` now maps each flag to its own threshold, the denominator
NAMES which one it is, and `test_no_firing_rate_can_exceed_one_hundred_percent` asserts
`successes <= trials` on every row of every flag — **with a control that something fires somewhere, so
the bound cannot hold vacuously.**

#### WHY THE FLAG NEVER FIRES, given that it could have

**Only rungs 1, 3 and 5 locate (`B129`, `B118`), and their cushions are far apart, so survivors' `rr`
values are far apart** — 2.72 and 8428 in the same row. **The widest survivor is the lowest-`rr` one,
and once the next survivor sits in the thousands, `argmin |rr - 3|` cannot prefer it.** So `GATE-030`'s
reachability, which criterion 4 established is governed by the SELECTOR rather than by monotonicity, is
in practice governed by **how far apart the surviving rungs are.**

> **The flag is not noise. It is silent, and it will stay silent until the ladder produces rungs whose
> cushions are close enough to compete** — which needs rung 2's declared momentum width and rung 4's
> producer.

#### And the mirror, which is the one that should be looked at

**`RR_ABOVE_ACCEPTABLE_BAND` fires on 84.3-100% of selections at every target in the range.** The
selected stop is above 4R almost always, so the ladder as the engine builds it over this corpus
produces stops that are **very tight relative to any plausible target.** `GATE-029` exists to flag
exactly that and is doing so almost always — **a flag firing on 84%+ carries little information per
firing**, the opposite failure to `GATE-030`'s, in the same pipeline and the same run.

**Bounds on all three figures:** ONE symbol, 5.2 days, one direction, a three-rung ladder, and an entry
placement and location that are ours and unratified (`B134`, `B128`).
...
#### THE 95% INTERVAL IS SAMPLING-ONLY, AND THE DEFINITIONAL AXIS WAS SWEPT SEPARATELY

**Review's finding, and it is the one that matters for what this number is used for.** A Wilson
interval quantifies SAMPLING uncertainty from `n = 521`. **It says nothing about DEFINITIONAL
uncertainty — and the definition of "a setup" is OURS.** A CI excluding 10% on sampling grounds alone
does not establish that the rate clears 10% under a different defensible reading.

**So the dedup rule was made a declared axis and swept.** All three arms come from ONE extraction, so
this costs nothing beyond the arithmetic:

    FIRST  (operative)   n = 529    20.3%  (106 of 521)   95% CI 17.1-24.0%
    LAST                 n = 529    20.6%  (108 of 525)   95% CI 17.3-24.2%
    NONE (every bar)     n = 555    20.5%  (112 of 546)   95% CI 17.3-24.1%

**The three cluster within 0.3 points and the LOWEST bound across all of them is 17.1% — still
seventy percent above the boundary.** So the conclusion does not rest on the choice, and that is now
measured rather than hoped. **Had one arm dropped near 10%, the finding would have been about my
dedup rule rather than about the ladder.**

**`NONE` is reported as an arm and never as the headline: it counts BARS wearing the word SETUPS**, and
that is the unit error this task was scoped on.
...
**THE OTHER HALF OF THIS ENTRY AS FIRST FILED IS A DUPLICATE AND IS WITHDRAWN.** I also reported that
the Manager's pin-3 control string `grep -c "rate AND rr against"` returns 0 — **Review found the same
thing about an hour earlier and it is `B92`'s third instance**, filed at `b58ff28`, with a corollary
better than mine: a transcription error and a genuinely broken control **present identically**, so
pasting is fault LOCALISATION and not merely error prevention. **Cite `B92`; there is nothing here I
add to it.**

*(Verified rather than accepted: `git show b58ff28:KNOWN_ISSUES.md | grep -n 'retype'` finds it at
`:3887`. The Manager's message quoted the string `paste the command, never retype it`, which is its
COMMIT SUBJECT and returns 0 against the file — the same retyping shape, one layer out, while
reporting it.)*

#### AND THE CONTAMINATION COMPOUNDS THROUGH THE ENTRY THAT REPORTS IT

**Measured after filing this: the count went from 4 to 8.** Four of the new hits are **THIS ENTRY**,
which had to write the token in order to explain that writing the token is the defect.

> **An entry documenting a contaminated control token contaminates it further, and the more carefully
> it is documented the worse it gets.** That is not a reason to leave it undocumented — it is the
> reason the fix cannot be "stop writing it" and has to be "never share one."

**So the rule is stronger than 'pick a distinctive token': a must-miss token must be UNIQUE TO ITS
USE — task id plus a nonce — because any token that gets discussed accumulates citations, and a
register that records its own control pairs is exactly the corpus that will accumulate them fastest.**
...
Manager, who upheld it against their own scope ruling.** **It is `B141`'s vocabulary lesson with the
roles reversed, and that reversal is the new part.**

#### 1. The probe that misleads is the one that comes back CORRECT

`T-0028`'s scope ruling established that `TARGET-005`'s mechanism already lived in `PRIM-004` and
re-cut the task to conformance. It then said **`TARGET-006` "IS UNAFFECTED AND REMAINS REAL BUILD
WORK … `PRIM-004` does not do that."**

    PRIM-004 does not do that.        TRUE.
    PRIM-003 does.                    prim_003_liquidity.py:70, :156, :181-186, COVERAGE_NOTE:125

**The ruling answered a question nobody needed answered** — the Manager's own phrasing on accepting
it: *"I checked the module seat 10 had named and concluded the rule was unbuilt, WITHOUT asking
whether any other module built it."* **So `B133`'s same-claim-two-homes trap was live on BOTH rules
of one task and had been closed on one.**

**THE INSTRUMENT WAS NOT BROKEN AND ITS ANSWER WAS NOT WRONG.** `grep RULE_ID = "TARGET-006"` returns
no file, and that is **TRUE**: no file claimed the id. **`B141` says a control pair validates the
instrument and never the vocabulary; this says a TRUE NEGATIVE on a correctly-chosen term can still
answer the wrong question — because "is this rule CLAIMED" and "is this rule BUILT" are different
questions and the id probe only ever answers the first.** A missing claim is what the coverage tool
measures, and this project has now twice found a fully built mechanism behind one.

**Practice:** before concluding a rule is unbuilt, probe the OBJECT's vocabulary across the whole
package, not the rule id, and print per-probe counts. What found it here:

    EQUAL_HIGHS_LOWS 10 · equals_class 6 · equal_highs_lows 2 · SEPARATE_POOLS 5
    RELATIVE_EQUALS_MAX_DIFF_PCT 2 · equals_diff_pct 5 · EQH 0 · EQL 1
    control zzz_T0028_absent 0            must-hit PRIM-003/PRIM-004 both claimed

#### 2. A declared enum value that nothing can produce, while the schema demands it

    prim_003_liquidity.py:65      SEPARATE_POOLS in the EqualsClass Literal      declared
    prim_003_liquidity.py:186     continue  # SEPARATE_POOLS ...                 the discard
    TELEMETRY_SCHEMA.json:544     in the equals_class enum                       declared
    RULE_REGISTRY.json:1964       TARGET-006's output names it                   required
    -----------------------------------------------------------------------------------
    ASSIGNED anywhere in the repository:  ZERO times

And `TELEMETRY_SCHEMA.json:550` asks for the opposite of what the code did: *"the measured difference
that produced `equals_class`. **Record it even when the class is `SEPARATE_POOLS`.**"* Over 54
windows of the pinned 5m fixture that discarded **519,046 measured differences spanning
0.3000%–3.2893% — the MAJORITY tier.**

> **THE DEFECT WAS EVIDENTIARY, NOT BEHAVIOURAL, and that distinction is what made the fix safe.**
> `continue` was the correct BEHAVIOUR — the two levels are already inventoried as swing levels and
> a third object would double-count. The classifier considered the pairing, rejected it, **and left
> no trace that it had looked.** FIXED: `equals_classification` returns all three tiers,
> `equal_highs_lows` builds pools from the two that have them, and the pool inventory is unchanged.

**A TYPE-LEVEL DECLARATION IS NOT COVERAGE, AND NOTHING LOOKS FOR THE GAP.** `SEPARATE_POOLS` was in
a `Literal`, in the JSON schema enum, and in the rule's `output` — three artefacts agreeing that the
value exists — while no code path could emit it. **Every one of those is a declaration; none is a
producer.** `check_rule_coverage.py` cannot see it (it counts rule ids), and schema validation
cannot see it (a value that never appears never fails validation). **Same blind spot as `B125`'s
producer-with-no-consumer and `B116`'s implementation-with-no-producer, one level lower: an ENUM
MEMBER with no assignment.**

#### 3. Two plan criteria were aimed at the halves that were already right

* **Criterion 5** — *"assert the objects returned differ in COUNT"* — has been asserted since before
  the task, `test_primitives_inventory.py:239-251`, all three tiers parametrised.
* **Criterion 2-i bullet 2** scored a CORRECT implementation as `FAIL`: it said a `NameError` on
  deleting the constant proves it is read, and deleting `WEAK_SWEEP_PENETRATION_PCT` **does** raise
  at `prim_004_sweeps.py:273` — the ANNOTATION line — because `TARGET-005`'s own `output` field
  requires the penetration and the ~1% band to be reported. **A rule that mandates a measurement
  cannot be checked by removing the measurement.** WITHDRAWN by the Manager; bullet 1 governs.
  **Both were found by RUNNING them, and neither was findable by reading.**

#### 4. Still open, and not this task's to fix

* **The quality boost has NO CONSUMER.** `boosters` is written by `PRIM-003` and read nowhere else
  under `app/` — zero read sites. So `TARGET-006`'s *"a booster, never a destination selector"*
  holds **because nothing reads it at all**, not because anything refuses to. Reported as `NOT_READ`
  with the producer named, never `TRUE`. `TARGET-006` therefore declares `CANNOT_FIRE_WITHOUT` and
  **cannot reach `PASS` today** — effective coverage moved 45 -> 46 while implemented moved 52 -> 54.
* **`GATE-038`'s parameter name diverges between registry and code.** The registry carries
  `amplifier_edge_tolerance_pct` with `_authority` and `_measured_from: NEAREST_EDGE`
  (`RULE_REGISTRY.json:992-995`); the code emits `amplifier_window_pct` with `_source` and **no
  `_measured_from` at all** (`gate_038_amplifiers.py:60-65`). The plan's `3-i` is right about the
  registry, which is what it cites. **Not fixed — another task's rule.**
* **`TARGET-005`'s `[ENGINEERING]` single-vs-cumulative requirement** was absent from the plan.
  *"Emit BOTH a per-bar maximum and an episode maximum, and declare which the flag reads."*
  **Two halves, and a record with ONE maximum plus a declaration naming it satisfies the second and
  not the first — while reading as complete, because the declaration is the visible half** (Review).
  Now both emitted. **MEASURED: 76,171 of 238,040 swept `(window, pool)` pairs carry an excursion
  whose weak/strong class disagrees with the episode maximum's — 32%.**

#### 5. `B142` caught this seat, in the same session it was filed

Both new `COVERAGE_NOTE`s were first written as *"nothing under `live/` imports it"* — `B142`'s false
form verbatim, hours after it was filed. **The first check ran against `backend/app/live/` and
returned 0 for EVERYTHING, including the must-hit arm.** A control pair where **both** arms miss is a
wrong path, not a clean result: the directory is `app/services/live/`. Re-run at the right target,
...
of three separate findings none of which is worth as much as the shape they share.** `B92` records this
disease at the **command** layer — *paste the command, never retype it.* **This is it at the REASONING
layer, and it had no entry.**

    THE MOVE   read the code, form a claim about what it does, publish the claim as a mechanism.
               The CONCLUSION is right. The MECHANISM named for it is not the live one.

#### The three instances, all 2026-08-17

    REVIEW    diagnosed T-0030's flatness test as dropping zero-opportunity rows, quoting
                  measured = [r.rate_over_opportunities.rate for r in rows if ...]
              The line committed at 0c1f32d was
                  measured = [r.rate.rate for r in rows if r.rate.rate is not None]
              -> the predicted path was ALREADY CLOSED, by a line it did not expect to find.

    MANAGER   claimed all four ladder locators referenced `target`, reporting 4 refs each.
              The `sed` range never terminated (the locators are NESTED functions), so it had
              measured one large block four times. Precise slicing: 0/0/0/0, risk_reward 1.
              -> would have CONTRADICTED Execute's correct finding, the one that saved 7a.

    EXECUTE   quoted a MEAN as evidence about a TAIL, on its own denominator.

**Right conclusion, wrong mechanism, every time — and every time the correction came from a seat that
EXECUTED it rather than read it.**

#### WHY IT IS DANGEROUS AND NOT MERELY UNTIDY — Execute's, and this is the load-bearing part

> **"Had I accepted your diagnosis as given, I would have 'fixed' a closed path and left the open one
> open — and the fix would have LOOKED CORRECT, because the verdict WOULD have moved on the mixed case
> for the wrong reason."**

**A fix aimed at a misdiagnosed mechanism can produce the expected symptom change while the real defect
survives.** The test goes green, the number moves the predicted way, and **the evidence that the
diagnosis was wrong is destroyed by the fix.** That is strictly worse than a wrong conclusion, which
someone eventually contradicts.

**And in this instance the surviving defect was worse than the diagnosed one, and neither seat had named
it:** the verdict was **computed on one denominator and PUBLISHED on the other** — `B127`'s pinned 50%
applied to a figure no reader ever sees.

    constructed corpus, 30 single-rung + 6 two-rung over [40,120]
      verdict on SELECTIONS    max 16.7%   all below 50%   -> REPORTABLE
      figure on OPPORTUNITY    0% - 100%   straddles 50%

**Found only because Execute read the committed line instead of the quoted one.**

#### The rule, and it is cheap

> **A mechanism claim is evidence only when you have RUN the code path, or quoted the line at the sha
> you are talking about.** *"I read it and it does X"* is a hypothesis. **`git show <sha>:<file>` costs
> one command and settles which line was live.**

**Corollary for the receiving seat, which is the half that actually caught all three:** **when a
colleague hands you a diagnosis, verify the MECHANISM before building the fix — not just the
conclusion.** Agreeing with the conclusion is what makes the wrong mechanism invisible.

**Not a criticism of any seat.** Reading is how all three of us understood the code well enough to be
...
was written to the platform.** Every figure below is from a `GET`, with a must-404 control confirming
the paths resolve.

    engine          running: true   paused: false   mode: PROP_FIRM_SIM   started 2026-08-14T11:02Z
    account         starting 5000.00   balance 5000.00   equity 4975.29   unrealized -24.71 (-0.49%)
    positions       open 2            closed_trades 0   wins 0   losses 0
    last TAKEN      2026-08-14T15:50Z
    latest bar      2026-08-16T23:05Z
    -> 55.3 HOURS, roughly 1326 bar-closes on the two symbols, EVERY ONE SKIPPED
    activity        40 of 40 entries are `skip`, all "already in a position, skipped"

**`SYMBOLS` is exactly two (`BTC/USD`, `ETH/USD`) and both hold a position, so `_has_position` blocks
every bar.** That is `T-0011`'s criterion `6a` state observed live: **the criterion passes on the defect
it exists to catch**, because with both pairs held a stale read cannot differ from a fresh one.

#### The alarm I did NOT file, because checking it took one command

**`/api/engine/decisions` returns 50 rows whose newest is `2026-08-14T15:50Z` — 55 hours stale. That is
NOT a defect and I nearly reported it as one.** The endpoint's own docstring: *"The decision log — **one
row per taken signal**"*. **No signal taken means no row written.** The staleness is the freeze's
shadow, not a second fault.

> **Same class as the `requirements-prod.txt` false alarm** — a confident negative about a colleague's
> working system, from an instrument I had not read the contract of. **Cost of checking: one `sed`.**

#### The shadow IS accruing, and that half is good news

    newest shadow evaluation   2026-08-16T19:05-04:00  ==  23:05Z   CURRENT, matches the latest bar

**So the fix that moved shadow evaluation ABOVE the entry gates works** — the contract layer keeps
reading live bars even while the trading side is frozen. **`B34`'s bias is genuinely mitigated on that
path.**

#### But the corpus it produces cannot validate anything

    n                    50
    decisions            {'STAND_ASIDE': 50}          ONE outcome
    deciding_rules       {'GATE-036': 50}             ONE decider
    rules_evaluated      4 distinct — GATE-023, GATE-008, GATE-007, GATE-002
    rules_blocked        EMPTY

**Fifty records that all say the same thing are `n = 1` seen fifty times** — the shape Execute found in
`setup_in_play` counting bars, and the one I found in the `x16` motif fixtures. **`GATE-036`
short-circuits to `STAND_ASIDE` before the rest of the roster is reached, so 4 rules are exercised out
of 49 implemented.**

**That is the implemented-versus-effective gap made concrete in production: we report 49 distinct rules
implemented, and production exercises 4.** **Neither figure is wrong; they answer different questions,
and only one of them is about whether the platform trades Salim's strategy.** **The endpoint says so
itself** — *"This is the answer to 'is the platform running Salim's strategy yet'. **It is not.**"*

#### TWO OPEN QUESTIONS, both stated as questions because I have not established either

1. **`rules_blocked` is EMPTY, and the endpoint's docstring says it should not be.** Verbatim: *"`blocked`
   is the part worth reading first… today that is the whole roster layer, because the correlate panels
   are not wired."* **Either the panels got wired — good — or the blocked counter stopped counting.**
   **Those are indistinguishable from this output**, which is this register's default failure appearing
   in the field meant to expose it. **Do not assume the good branch.**

   > ## `[RESOLVED 2026-08-17 — IT IS THE GOOD BRANCH. THE CORRELATE PANELS ARE WIRED AND THE DOCSTRING IS STALE.]`
   >
   > **Settled by reading what `rules_blocked` counts rather than by assuming either branch:**
   >
   >     the endpoint    if ev["verdict"] == "NOT_APPLICABLE": blocked[rule_id] += 1
   >                     else:                                 evaluated[rule_id] += 1
   >     shadow.py       the layout-UNREADABLE branch appends GATE-002 with verdict="NOT_APPLICABLE"
   >     production      rules_blocked EMPTY · GATE-002 in rules_evaluated 50 of 50
   >     => GATE-002 was never NOT_APPLICABLE => THE LAYOUT WAS READABLE ON ALL 50 RECORDS
   >
   > **And the panels are visible in the record:** `BTCUSDT.P: 8639 bars from https://fapi.binance.com
   > (PERPETUAL, symbol BTCUSDT)`, same for `ETHUSDT.P`. **All four panels present, `GATE-008` PASS,
   > `GATE-002` grading four real panels.**
   >
   > ### THE TRAP THAT COST ME THREE REVERSALS — the two panel loops LOG ASYMMETRICALLY
   >
   >     dominance success   PanelFetch(roster_name, frame, samples)                    NO NOTE
   >     perpetual success   PanelFetch(roster_name, frame, None, "…8639 bars from …")   NOTE
   >
   > **So a record whose `why` names only the perpetual panels is a record where all four SUCCEEDED.**
   > I read the missing `TOTAL`/`USDT.D` notes as absence and concluded the dominance half was not
   > arriving — **twice, in opposite directions, before checking what a success actually writes.**
   > **A silent success and a silent failure are the same string.**
   >
   > **`STAND_ASIDE` on 100% of bars is therefore CORRECT OUTPUT, not a defect.** A `TAKE` additionally
   > requires `stop_evaluation`, `target_selection` and `entry_criteria`, and **`entry_criteria` does not
   > exist before M6** — `shadow.py:689`. **The shadow stands aside because the entry layer is unbuilt,
   > which is the honest answer.**
   >
   > **CONSEQUENCE FOR PLANNING, and it killed a task before it was written:** *"wire the correlate
   > panels"* was recommended to Malek as the highest-leverage next step, **on the strength of this
   > docstring and a stale register line, without measuring.** **There is no such task. It is done.**
   > **The stale docstring should be corrected by whichever task next touches `engine.py`** — not
   > retro-edited here, since the sentence was true when written.
2. **Two positions have been open 55 hours with zero closes, and NOBODY CAN SEE THEIR STOPS.** `B67`:
   no endpoint publishes a position's `sl`/`tp`. **`-24.71` across two positions on a 1%-risk sizing is
   consistent with price simply not having reached either stop, and equally consistent with stops that
   are wrong** — **and from outside the two cannot be distinguished.** `/api/positions` returns `[]`
   because it reads `broker_manager`, which has no registered broker, while the engine's count comes
   from its own sim broker: **"no broker connected" and "zero positions" are the same output.**

#### Why this is filed rather than mentioned

**`B34` records that *"already in a position" is its normal state*; this entry records what that costs
measured in production: 55 hours, ~1326 skipped bar-closes, 4 rules exercised, one outcome.** **Nothing
here is a new defect class.** **What is new is the magnitude, and that the frozen state has no exit
visible from any endpoint.**

Related: **B34**, **B67**, **B31**, **B116**, and `T-0011`'s criterion `6a`.

### B127 — the target sensitivity sweep needs its FLATNESS criterion pinned before it runs, or "unmeasured, but stable" becomes a paragraph instead of a measurement

**Filed by the Manager 2026-08-16, ruling on Execute's pre-build escalation for `T-0030`. Execute
asked for this to be attacked before its diff existed, which is why the entry exists at all.**

#### The situation, which is a plan defect of mine

**`rr = |target - entry| / |entry - stop|`, and NOTHING IN THE ENGINE PRODUCES A TARGET.**
`LadderInputs.target` is an input by design — *"so that a stop can never be chosen by moving the
target"*. `TARGET-001` needs an `InstitutionalDestination` whose own docstring reads *"AN INPUT —
nothing in the engine produces one"*, and `shadow.py:692` says a TAKE requires `stop_evaluation`,
`target_selection` and `entry_criteria`, *"none of which exists before M6"*.

    criterion 7a   monotonicity inversion rate   reads CUSHION |entry-stop|   SURVIVES INTACT
    criterion 4b   TIGHTER_THAN_NECESSARY rate   consumes rr                  UNMEASURABLE
    criterion 5b/7b  the eps firing rate         consumes rr                  UNMEASURABLE
    criterion 6b   corpus rate (the FIXTURE is fine)                          UNMEASURABLE

**Verified independently: all four ladder locators contain ZERO `target` references and
`risk_reward` contains one** — measured by slicing each function precisely, after a first attempt
using a `sed` range over-captured and reported `4` for all four, **which is the tell that the
instrument rather than the code was wrong.**

#### The ruling: NO target proxy. Sensitivity instead. And here is why they are not the same shape

**Execute identified a defensible middle option and correctly did not take it:** extend `7a`'s
labelled-proxy grant — `detect_order_blocks()`, permitted for MEASUREMENT, forbidden for SELECTION —
to a target via `TARGET-003.select_within_tf`. **REFUSED, and the distinction is the point:**

    the order-block proxy   supplies a CANDIDATE'S PRESENCE. The question is structural — CAN the
                            interior ordering invert — and the proxy's value does not enter the
                            answer arithmetically.
    a target proxy          sets the DENOMINATOR OF EVERY rr, which IS the measured quantity.
                            Its value propagates straight into the reported rate.

**And `TARGET-003`'s own docstring calls collapsing the three destination levels *"the defect"*** — so
the proxy would make a known defect the basis of a headline number. **A proxy that supplies a point is
not a proxy that supplies the scale.**

#### THE PINNED CRITERION, because "flat enough" is otherwise decided after the data is seen
...
target was; a sweep that crosses it means the answer is an artefact of a number nobody produced.

**This is `B124`'s mechanism applied a second time:** the threshold is pinned by the seat that does NOT
measure, in a commit that predates the work, **read from the pin rather than from `HEAD`:**

    git show <this commit>:KNOWN_ISSUES.md

**If a later seat disagrees, argue with the 50% and the decision behind it — do not move it after
seeing a sweep.** Revising it requires a NEW pinned commit and an amendment to the criterion citing it.

#### And three things that make the sweep evidence rather than prose

**Execute's own stated worry is the right one:** *"'report the rate as UNMEASURED and measure
sensitivity instead' is the kind of move that can quietly become 'I measured nothing and wrote a
paragraph'."* **So:**

1. **The swept range must be DERIVED and stated** — what makes a target plausible, and why those
   bounds. **An arbitrary interval makes the flatness result arbitrary too.**
2. **Report a TABLE of rate against target, not a verbal summary.** A reader must be able to see the
   crossing or its absence without trusting the conclusion.
3. **Name the input that would make the sweep report TARGET-DEPENDENT** — per the default-failure
   section's fix 3. **If no such input can be named, the sweep is an assertion.**

---

### `[AMENDED 2026-08-16 BY REVIEW — THIS IS A NEW PIN. The 50% criterion above is UNCHANGED; three requirements are ADDED. Cite THIS commit, not the first one.]`

**Per this entry's own rule — *revising it requires a NEW pinned commit and an amendment to the
criterion citing it* — the first pin `4a2ed17` still holds the unamended text and is superseded rather
than edited.** `T-0030`'s criteria `4b` and `7b` now cite this commit.

#### 4. THE RANGE IS MEASURED FROM THE CORPUS, NOT PINNED BY ME

**I proposed pinning the range too. Review refused it on this entry's own reasoning, and is right:** I
cannot derive a plausible-target range without the data either, **so pinning one would just move the
arbitrariness** — `B124`'s argument turned back on me.

**REQUIREMENT: derive the range from the corpus and state the derivation** — the observed distribution
of candidate target distances, min to max, measured. **Then the range is a measurement a reviewer can
recompute rather than a preference to be argued with.**

#### 5. THE RANGE IS PART OF THE FIGURE'S NAME, NOT A CAVEAT BESIDE IT

    "the rate is 12%"                              a claim about an unmeasurable quantity
    "across targets in [a, b], the rate is 12%"     measurable

**They differ by a clause, and the clause disappears on the first requote.** **That is `54 corpora` →
`54 setups` exactly: a real number carried out of the scope that made it real.** **So the requirement is
not "state the range" — it is that anything quotable without the range must not exist in the report.**
**Anything that CAN be quoted without it will be.**

#### 6. THE RESIDUE NEITHER OF US NAMED: A FLAT SWEEP MAY MEAN THE SWEEP WAS INERT

**The 50% criterion tests whether the RATE crosses. It does not test whether the SWEEP COULD HAVE MOVED
IT.**

    flat because the rate is genuinely robust across plausible targets   -> REPORTABLE, correctly
    flat because every swept target yields large rr and the flag never
      fires anywhere in the range                                        -> ALSO PASSES, proves nothing

**Both produce a clean table, a flat line, and one side of 50%. They are indistinguishable from the
output, and a reader defaults to the flattering one.**

> **That is fix 3 of the default-failure section — committed at `08f5d88` — turned on the instrument
> fix 3 was written for.** *A check is only evidence if you can state the input that would make it
> fail.* **Applied here: state the input that would make the sweep MOVE.**

**REQUIREMENTS, both checkable from the table this entry already demands:**

* **Report `min(rr)` and `max(rr)` across the swept range.** **If `rr` barely varies, flatness is a
  property of the sweep rather than of the rate.**
* **Require that SOMETHING crosses a threshold somewhere in the sweep** — a candidate crossing the 2R
  floor, or the selected rung changing. **If no intermediate quantity changes anywhere in the range, the
  sweep is INERT and reports UNMEASURED regardless of how flat it is.**

**That is what makes "flat" mean anything.** Without it, **the most inert possible sweep produces the
most reassuring possible result** — and that is the direction that always wins, **because nobody
investigates a clean answer.**

---

### `[AMENDED AGAIN 2026-08-16 — THIRD AND CURRENT PIN. Requirement 7 closes the hole requirement 6 left open. The 50% threshold is STILL unchanged.]`

#### 7. PUT `rr` IN THE TABLE. Inertness becomes VISIBLE PER ROW instead of asserted globally

**I left a hole open and said so rather than half-closing it:** requirement 6 says *something* must cross
a threshold **somewhere** in the sweep, **but not that the crossing lie in the region the rate is read
from.** So a sweep could cross the 2R floor at one extreme, satisfy requirement 6, **and still be inert
across the interval that produced the reported figure — an active extreme laundering an inert middle.**

**Review's fix needs no new number, which is why it is the whole fix:**

> **Requirement 2 already demands a table of rate against target. Make it rate AND `rr` against
> target.**

    target      rr        firing rate        <- rr is the column that was missing
    t1          2.41      18%
    t2          2.41      18%               <- SAME rr: t1 and t2 are ONE experiment run twice
    t3          2.88      22%

**THE PER-ROW RULE: `rr` must differ between every adjacent pair of rows.** **Identical `rr` on two
consecutive rows means those two targets are the same experiment**, and a sweep of duplicates is inert
however wide its endpoints are.

**Why this closes it completely rather than moving it.** The claim being licensed is *"across `[a,b]`
the rate stays on one side of 50%"* — **a claim about every point in the range.** A table carrying `rr`
per row **supports or refutes it point by point**, so **there is no aggregate left to be inert about:
the criterion is read off rows, and a row that did not move is a row that says so.**

> **`B121`'s move applied here.** Requirement 6 asks whether the sweep moved; **putting `rr` in the
> table makes that an OBSERVABLE rather than a claim** — the same reason `B121` diffs the emitted record
> instead of counting reds.

**And it relocates any future argument.** A later seat disputing the finding now argues **with a visible
row** rather than with a summary sentence — **which is why requirement 2 replaced a verbal summary with
a table in the first place.**

Related: **B124**, **B116**, **B122**, and `T-0030`'s criteria 4b / 5b / 7b.

### B126 — the calendar's impact default fails OPEN, twice over, and the direction is toward trading during news

**Found by Review 2026-08-16 scoping `T-0031`; verified independently by me, which turned one
fail-open into two stacked ones.** `GATE-015`'s filter is a **safety** filter — its job is to make the
engine stand aside — **so the direction its defaults fail in is the whole question.** Round-2's hard
rule 5, in the subsystem where it costs the most.

    finnhub.py:262   impact_raw = str(item.get("impact") or item.get("importance") or "low")
    finnhub.py:263   impact     = _IMPACT_MAP.get(impact_raw.lower(), "low")
    finnhub.py:188   if event.impact != "high": continue        <- "low" is SKIPPED

**TWO INDEPENDENT ROUTES TO `"low"`, and neither is a typo:**

    route 1   the API omits BOTH `impact` and `importance`   -> impact_raw = "low"
    route 2   the value is present but UNRECOGNISED          -> _IMPACT_MAP.get(...) = "low"

> **Either way the event becomes `"low"`, `"low"` is skipped by the filter, and an event we could not
> classify is treated as harmless. The trade is TAKEN.**

**This is not a neutral gap. It is a systematic tilt in one direction** — the same shape as `B103`'s
main-asset exclusion — **and the tilt is toward acting during news rather than standing aside**, which
is the exact behaviour `GATE-012`/`013` exist to prevent.

#### It is dormant as a GATE and live as DATA, and those are different blast radii

**The blackout consumer is unwired (`B125`), so nothing trades differently today.** But
`finnhub.py:262-263` sits in the **fetch/parse** path, which **is** live: the hourly cron refresh at
`main.py:254-262` and the router at `GET /calendar/today`. **So an event whose impact we cannot read is
already being SERVED to the UI as `"low"` — a real event displayed as unimportant.**

**And it compounds `B125` exactly:** the moment someone wires the blackout in, **this default decides
which events are safe**, having been chosen in a default argument that no review ever looked at.

#### The fix is to stop deciding a safety question in a default argument

* **An unrecognised or missing impact becomes `UNKNOWN`, never `"low"`.** The two are different facts
  and one boolean cannot carry both — **this register's standing failure in a two-state field.**
* **The handling of `UNKNOWN` is a DECLARED, RECORDED choice** — block on unknown, or trade on unknown
  — **stated in the rule's telemetry rather than implied by a default.** `.get(..., "low")` answers
  *"should we trade through an event we do not understand?"* **silently, in punctuation.**
* **Count how often it fires** before choosing. If `UNKNOWN` never occurs against the live feed the
  choice is cheap; if it fires often, blocking on unknown may halt the engine regularly. **Neither is
  knowable without the count**, and `T-0031`'s `1b` requires it.

**Not fixed here** — `T-0031` owns it, criterion `1a`. **Filed now because the entry must exist before
the seat that fixes it decides how, and because the two-route version is harder to see than the one
Review reported.**
...
>
> **My framing read as though the false alert fires today. It does not fire at all** — the branch is
> `if candle.get("news_blackout"):` and nothing sets the key, so it is dead. **That is not the softer
> version:**
>
> > **Setting `news_blackout` is the natural FIRST step of wiring the calendar in. Enforcing the block
> > is separate work.** So the moment someone populates the key — **the obvious half-hour change** —
> > the platform starts telling users *"trading suspended"* while taking every trade.
>
> **The defect activates on the first half of its own fix, and that half is the part that looks safe
> because it only populates a field.** Anyone doing that wiring would reasonably expect to have changed
> nothing behavioural.
>
> ### THE RULE THAT FOLLOWS, and it is the actionable half of this entry
>
> > **Populate `news_blackout` and enforce the block in the SAME change, or populate neither.**
>
> **"Unwired decision" invites a fix that sets the key and stops** — which is precisely the edit that
> turns a dormant false claim into a live one. **Verified by Review:** `is_in_blackout` callers are
> `finnhub.py:110 -> finnhub.py:161`, inside its own module only; `news_blackout` has exactly one
> occurrence in all of `app/` and it is a read.

**Found by the Manager 2026-08-16, scoping what remains after `T-0030`. Every claim below carries a
control pair, because the last time I reported an absence here I was wrong (`B122`).**

**The economic calendar is REAL AND RUNNING. It is not dead code and I nearly filed it as such:**

    app/services/calendar/finnhub.py   CalendarService, CalendarEvent, is_in_blackout()
    app/main.py:106-108                calendar_service.init(api_key=…, redis_client=…)   AT STARTUP
    app/main.py:254-262                scheduler.add_job(_refresh_calendar, "cron", minute=5)   HOURLY
    app/main.py:476                    app.include_router(calendar_router, …)
    app/api/routers/calendar.py:12     GET /calendar/today                                 SERVED

**What is unwired is everything that would ACT on it:**

    is_in_blackout()              never called outside its own module
                                  CONTROL must-hit: generate_risk_warning has 2 call sites
    candle["news_blackout"]       ONE occurrence in all of app/ — a READ, at engine.py:173.
                                  Nothing anywhere SETS the key, so the branch cannot fire.
    what the branch does          appends a risk-warning alert. It does not return, raise,
                                  skip or reject — CONTROL: grep for return/raise/continue
                                  within 3 lines of it returns empty.

**And the alert's own text, verbatim at `engine.py:179`:**

> *"High-impact news event imminent — **trading suspended**"*

**Nothing is suspended.** The string names an action the surrounding code does not perform, **and it is
the sentence a reader trusts** — `B93`'s shape, in a user-facing alert rather than a test message.
**If the key ever starts being set, the platform emits "trading suspended" and keeps trading.**

#### Why this is worth an entry now rather than when the news rules are built

**Because the five rules that GOVERN news handling are all unimplemented** — `GATE-012`, `GATE-013`,
`GATE-014`, `GATE-015`, `GATE-016`, five of the thirty distinct `HARD_GATE`s remaining — **and no rule
module consumes the calendar at all** (`grep -rnE "^\s*(from|import)\s+.*calendar" rules/` → empty;
**CONTROL: the same pattern finds the imports in `gate_023_timezone.py`**).

**So this is `B123`'s family with the polarity that matters most:** not behaviour without a rule, but
**a live producer, a written-but-unreachable consumer, and five rules that would govern it sitting
unbuilt.** `B41`'s species — *a detector that was never wired* — except the data half **is** wired and
only the decision half is not.

> **The good news is the planning news.** The recurring blocker on this programme is rules that cannot
> fire for want of a producer (`B116`, four instances). **`GATE-012`–`016` are NOT in that bucket:**
> the producer exists, runs hourly, and is populated. **They are buildable whenever a task takes them.**

#### What a task must NOT do here

* **Do not treat `engine.py:173` as the specification.** It is pre-contract code, no rule authorises
  it, and adopting its behaviour would install our blackout semantics as Salim's — **the move `T-0029`
  refuses for order blocks.**
* **Do not fix the alert string in isolation.** Correcting the wording to match a branch that fires on
  a key nobody sets **makes the record read as reviewed while leaving the gap** — and whether the
  engine SHOULD suspend on high-impact news is `GATE-013`/`GATE-015`'s question, not a copy edit.
* **Do not delete the dead branch either.** It is the only evidence in the tree of what the
  pre-contract system intended to do about news, **and `T-0029`'s treatment of `ict/detector.py` is the
  precedent: characterise it as input to the question, adopt nothing.**

#### THIS IS `B116`'s EXACT INVERSE, and both are invisible to the same instrument — Review's framing

    B116   implementations WITHOUT PRODUCERS   the rule cannot fire; coverage OVERSTATES
    B125   a PRODUCER WITHOUT CONSUMERS        the data is live, populated, hourly, and
                                               nothing acts on it

**Every instrument this loop has counts them identically, and neither is visible to
`check_rule_coverage.py`** — which measures whether a rule id **appears in source**, not whether
anything **reaches** it, nor whether anything it **produces is read.** **Two opposite defects, one blind
spot, and the blind spot is in the tool we quote coverage from.**

Related: **B41**, **B93**, **B116**, **B123**.

### B124 — "declare the floor before you see `n`" is a claim about a mental state, so the artefact cannot carry it — and THIS ENTRY IS THE FIX, not a description of one

**Found by Review 2026-08-16, reviewing `T-0030`'s plan. I asked it to check whether a seat could
write a low floor after the fact. The answer is that the criterion could not bind at all.**

`T-0030`'s criterion 2 required the measuring seat to **declare a minimum `n` BEFORE measuring**, so
that a corpus too small to carry a rate gets reported as `n` rather than as a percentage.

**Review's finding, and it is worse than the hole I expected:**

    commit ordering does not establish it   compute n locally, commit the floor first and the
                                            measurement second — the history is IDENTICAL to honest order
    genuine ordering does not either        an ordered pair of commits does not prove the seat had not
                                            already run the harness

> **The criterion asks about a mental state at a moment, and no artefact can carry that.** As written
> it is a **self-report** — `B116`'s shape one layer up. **The bucket is honest exactly to the extent
> the implementer is**, which is the property this register exists to remove.

#### The fix is to take the number away from the seat that measures

**Review's, and I am applying it here rather than recording it.** `B82` — *a remedy left as prose in an
entry has no owner and no trigger* — so the floor is **in this entry, committed by the Manager, before
any Execute seat branches for `T-0030`.** Then:
...
the unverifiability Review found.

#### THE FLOOR, AND HOW IT WAS CHOSEN — a method, because a round number just moves the arbitrariness

> ## `n >= 30 setups` for either rate to be reported as a rate at all.

**Derived from what the measurement has to decide, not picked.** The live decision is whether real data
can supply a conformance case for `GATE-028`'s tie-break, or whether a constructed fixture is required
regardless. **A constructed one is required whenever inversions are rarer than roughly one setup in
ten**, so **10% is the boundary where the answer changes.**

**By the rule of three, observing zero events in `n` trials puts the 95% upper bound at about `3/n`:**

    n = 30   a zero result bounds the true rate at <= 10%   <- the smallest n that can exclude the
                                                               rate that would change the decision
    n = 60   <= 5%
    n = 100  <= 3%

**So below `n = 30`, "0 of n" is consistent with a rate large enough to have changed the answer, and
the measurement cannot inform the decision it exists for.** That is the floor's whole justification —
**if a later seat disagrees with the 10% boundary, it should argue about THAT and re-derive, not nudge
the 30.**

#### A second mechanism, and it is the stronger of the two

**Require a 95% interval on every reported rate** (Wilson, or any stated method). **Then `n` travels
INSIDE the number and cannot be hidden behind a percentage** — *"33% (2 of 6, 95% CI 6–76%)"* refuses
to be read as *"33%"*. **A floor is a threshold someone can argue past; an interval attached to the
figure is not.** `B80` — a note is not a mechanism.

#### CORRECTED SAME DAY — the windowing instrument EXISTS, and the floor is about SETUPS not WINDOWS

**This entry's first version inherited `B122`'s false premise that no corpus existed. It does** —
`test_t0017_threshold_over_corpora.py`, `CORPUS_BARS = 863`, `STEP_BARS = 12`, **54 sliding windows**
over `btcusdtp_5m_1500.csv`, producer `validate_over_corpora()` at `consolidation.py:270`. **See
`B122`.**

**The floor is unaffected, because it was always about the wrong quantity to inherit:** `n >= 30`
governs **SETUPS**, and **54 WINDOWS is not 54 SETUPS.** The setup denominator remains unmeasured, and
it is what `T-0030`'s criterion 1 must produce.

**One thing the existing instrument makes possible that the plan did not ask for:** the 54 windows are
a **resampling structure**. An inversion rate computed per window yields **54 estimates rather than
one point**, so the spread is observable directly instead of via an interval formula. **That is a
better answer than a single rate and it costs nothing extra** — the windowing already exists.

#### The outcome this pre-commits, which is the point of pre-committing it

**`btcusdtp_5m_1500.csv` is 1500 bars of ONE symbol over 5.2 days, and the 54 windows OVERLAP heavily
(863 bars, step 12) — so they are not 54 independent samples of anything.** **It may well yield fewer
than 30 setups**, in which case **both of `T-0030`'s headline measurements are UNDELIVERABLE from the
data we hold**, and the honest output is `n`, the floor, and a `NEEDS_MALEK` item about corpus data.

**That is now a pre-committed outcome rather than a judgement made after seeing the number** — which
is the entire difference between this and criterion 2 as I wrote it.

Related: **B116**, **B82**, **B80**, **B122**.

### B122 — a REAL number carried across a unit boundary survives every check for fabrication — and the audit that went looking for it read an empty glob as absence

> ## `[CORRECTED 2026-08-16 BY EXECUTE, SAME DAY, BEFORE THE INK DRIED. Heading and thesis both replaced.]`
>
> **The first version of this entry claimed "there is no corpus at all". THAT WAS FALSE, and the
> evidence for it was a one-letter-wrong glob.** The corpus exists:
>
>     find -name "*corpus*"    ->  EMPTY      <- what the entry rested on
>     find -name "*corpora*"   ->  backend/tests/unit/test_t0017_threshold_over_corpora.py
>
> **CorpoRA. Latin plural.** `CORPUS_BARS = 863`, `STEP_BARS = 12`, `EXPECTED_CORPORA = 54`, producer
> `validate_over_corpora()` at `app/services/rules/consolidation.py:270`, harness green at 14 passed.
> **Verified three ways** — Execute ran the suite and drove the real function; I confirmed the
> arithmetic independently of the constant: `len(range(0, 1500-863+1, 12)) == 54`.
>
> **So an entry about promoting unverified claims was filed on an unverified negative of its own**, by
> the seat that had already logged a false alarm from a guessed path earlier the same day. **That is
> not an aside — it is the best instance this entry has, and the heading now says so.**

**Filed by the Manager 2026-08-16, corrected by Execute within the hour. `B120`/`B121` record
Execute's own instance, which is unaffected and still the cleanest statement of the citation half.**

#### What was actually wrong, which is smaller and sharper than what was filed

    "54 CORPORA"  T-0017's sliding windows over btcusdtp_5m_1500.csv.  REAL. It is the instrument.
    "54 SETUPS"   a monotonicity denominator.                          DOES NOT EXIST.

**`T-0025`'s `1b-i` wrote *"Monotonic in 54 of 54 setups"*.** The `54` was **borrowed from the real
harness** — `1b-i` explicitly invokes *"the `B78`/T-0017 shape"*, and T-0017's own summary reads *"in
bounds on 54 of 54 corpora"*. **Grounding an illustration in the instrument that would run it is good
practice, not invention.** What went wrong is one word: **the number of WINDOWS in a corpus is not the
number of SETUPS in it**, and the illustration silently equated them.

**`T-0029`'s *"the 54-window corpus"* was therefore CORRECT**, and Execute's handoff figure was
**measured independently, not inherited from me** — so the three-hop chain the first version drew does
not exist. **One hop, one unit.**

> **A real number carried across a unit boundary is harder to catch than a fabricated one, because it
> survives every check for fabrication.** Provenance checks out. Arithmetic checks out. Only the noun
> is wrong. **That is `117 vs 104` in a new place** — and `1b-iv`, three sections below the defect,
> exists to prevent exactly it: *"the rate must carry its unit."*

**REVIEW'S FRAMING, WHICH IS SHARPER THAN MINE AND IS THE REASON THIS ENTRY IS WORTH KEEPING:**

    every earlier instance in this register   ONE QUANTITY wearing SEVERAL COUNTS
                                              117 vs 104 · 91 vs 79 · 24 files vs 19 · 26 vs 27
    this one                                  ONE COUNT wearing TWO QUANTITIES
                                              54 windows / 54 setups

**Opposite surface, same root — and no existing guard looks for it.** Every instrument this loop has
built checks whether **two numbers of the same thing agree.** **None checks whether one number is
being used for two things**, which is undetectable by comparison because there is only ever one figure
to compare. `B84`'s corollary in reverse: *two numbers that agree by coincidence are expensive* —
**here there is a single number agreeing with itself.**

#### The second failure is the serious one, and it is this register's default

**Auditing my own number, I ran `find -name "*corpus*"`, got nothing, and concluded the instrument was
fictitious.** No control pair. **An empty result is a claim and needs the same evidence as a positive
one** — Execute's predecessor filed the same shape (`grep -rlE "a\|b"` with a literal pipe under `-E`,
empty read as *"no producer exists"*), and it is item 1 of its handoff.

**The correct instrument is the one Execute used and I did not:**

    CONTROL must-hit   find -name "test_t0017*"    -> 2   the search CAN find things
    CONTROL must-miss  find -name "*zzz_absent*"   -> 0   and CAN return empty

**Neither control costs anything. Without them, `0` and `0` are indistinguishable.**

#### The fixes, and both survive the correction

1. **Execute's, for citations: open the entry before citing it. It costs one grep.** *"That is Bnn"*
   in any handoff, verdict or work report **is an analogy unless it quotes the entry.**
2. **Write hypothetical figures so they CANNOT be mistaken for measurements — `N of N`, never a
   plausible value.** **Execute's argument for keeping this unchanged is that it also fixes the unit
   hazard:** *"N of N setups"* cannot borrow a corpus count, because there is no number to borrow.
   `B80` — a note is not a mechanism; this is the mechanism.
3. **NEW, from the second failure: a negative result gets a control pair or it is not evidence.**
   Every `find`/`grep` whose EMPTINESS is load-bearing runs beside a must-hit and a must-miss **in the
   same command**, and both appear in whatever the finding is written into.

**Nothing is retro-edited.** `1b-i`'s illustration and Execute's `review-01.md` analogy were both
defensible where they stand; **editing origins makes chains unreconstructable, which is the only
reason either was traceable.** Corrections live at the hops that promoted them — **including this one,
which is a correction to a correction and is kept visible for that reason.**

**The `T-0030`-before-`T-0029` ordering is UNAFFECTED and stands.** It was justified on the harness
existing so that `T-0029`'s criterion 5 becomes a second query rather than new code — **and the harness
exists, runs green, and returns 54.**

Related: **B80**, **B93**, **B120**, **B121**, **B124**.

### B121 — an anchor assertion proves a mutation was APPLIED, never that it was VALID — and an invalid one over-fires, which reads as thorough coverage

**Found and measured by Review, 2026-08-16, verifying T-0025a's mutation M4. Filed by Execute
because the tree was already dirty on this seat — finder is not committer, and the two-seat
property holds either way.**

**THIS ENTRY IS A CORRECTION TO ADVICE THIS LOOP HAS BEEN GIVING ITSELF ALL DAY**, which is why it
is filed above the findings it corrects.

**The standard anti-vacuous-mutation guard is to anchor on a count** — `assert source.count(old)
== 1` before replacing — so that a `sed`/`replace` matching nothing cannot pass as a mutation that
fired. Good advice, given twice today. **It does not cover this class at all.**

Review applied M4 three times. **The anchor assertion PASSED on all three.** The text matched, the
replacement landed, the file changed:

    attempt 1   anchor matched · replaced · raw dict where SearchEvidence required   ->  7 failed
    attempt 2   anchor matched · replaced · wrong field names, TypeError             -> 23 failed
    attempt 3   anchor matched · replaced · correct                                  ->  1 failed

**Every mutation was correctly APPLIED. Two were semantically INVALID** — they crashed the code
under test rather than changing its behaviour — **and both printed a clean pytest summary with a
plausible failure count and no indication anything was wrong.**

> **The anchor count proves the mutation REACHED THE CODE. Only a diff of the emitted record
> proves it MEANT WHAT YOU THOUGHT IT MEANT. Neither substitutes for the other, and the second is
> the one nobody runs, because the pytest summary already looks like an answer.**

**AND THE DIRECTION OF THE ERROR IS WHAT MAKES IT DANGEROUS.** Both invalid attempts produced
**MORE** failures than the correct one, not fewer:

    a mutation that UNDER-fires    reads as a coverage gap        -> prompts investigation
    a mutation that OVER-fires     reads as thorough coverage     -> prompts a satisfied nod

**`7 failed` and `23 failed` both look BETTER than the truth of `1 failed`.** A seat reporting
*"M4 fires 7 tests"* would have been believed, would have been wrong, and would have been claiming
strength it did not have. **The flattering artefact is the one nobody audits.**

**The pair, and run both:**

    anchor count    the mutation reached the code
    record diff     the mutation meant what you thought it meant
...
**Filed by Execute 2026-08-16, T-0025a. Found while checking whether a ruling could be
implemented; the ruling was withdrawn as a result, and this outlived it.**

**`$defs/stop_candidate` in `TELEMETRY_SCHEMA.json`:**

    required   ['rung', 'anchor', 'locatable', 'accepted']

**Neither `unlocatable_reason` NOR `search_evidence` is in it.** Meanwhile three separate
descriptions in the same file state that they are mandatory:

    unlocatable_reason.description  "Closed set of REAL search failures. Each value requires
                                     `search_evidence`, so an auditor can contradict the claim
                                     from the record alone."
    search_evidence.description     "Required whenever `locatable` is false…"
    locatable.description           "HG-28 therefore requires the reason to be a closed-enum
                                     value backed by `search_evidence`."

**So an engine that emits `locatable: false` and nothing else PASSES SCHEMA VALIDATION**, and
the only thing standing between the ladder and a silently truncated one is that nobody has
written that code yet.

**Why it matters more than a missing `required` entry usually would.** `locatable.description`
says it itself: *"false is a legitimate and important value, but it is also the cheapest way to
truncate the ladder until only the engine's preferred rung survives, after which the GATE-028
selector trivially agrees with itself."* **The excuse-free skip is the exact attack HG-28 was
written to prevent, and the schema does not mechanically prevent it.**

**This was found from the inside, by the seat that would have benefited.** T-0025a needed a way
to record rung 4's producer gap; emitting `locatable: false` with neither field would have been
schema-valid, cost nothing, and been undetectable. **It was refused and the gap filed instead.**
`StopCandidate.__post_init__` in `gate_027_stop_ladder.py` enforces the prose invariant in code
for the rungs this engine emits — **which protects one producer, not the record type.**

**NOT FIXED HERE: Execute may not edit the telemetry schema.** Escalated to the Manager, who
recorded it for the same contract round as the other schema questions. Related: **B107** (the
schema changed content with no version bump), **B110**.

### B118 — the workspace's printed stop ladder is in the WRONG ORDER, and it is the more findable source

**Filed by Execute 2026-08-16, T-0025a. Required by the plan's own record list.**

**`GATE-027.values.ladder` is authoritative and is the ENGINE order:**

    1 DEEPEST_SWING   2 MOMENTUM_IMBALANCE   3 LIQUIDITY_SWEEP_QML   4 ORDER_BLOCK   5 INNER_MSB

**The workspace (`§2 H8`, `§3.5`, `§8`) runs ORDER BLOCK THIRD and hunted-liquidity/QML FOURTH.**
The registry says outright that this is *"superseded for execution"*.

**Why this is filed rather than merely commented:** rungs 3 and 4 are the two the orders disagree
about, and **they are also the pair `B83`'s motivating example is about** — a swept wick sitting
closer to entry than an order block. **Anyone checking that example against the workspace gets
the rungs the wrong way round**, and the workspace is the document a human reaches for.

**Mitigated, not fixed:** `test_order_block_is_rung_four_not_rung_three` pins the difference, and
the module reads the order from the registry rather than retyping it (mutation M8: retyping it
...
### B119 — four more rules that can reach a verdict and that nothing calls

**Filed by Execute 2026-08-16, T-0025a. Same shape as B115; filed separately because the count
moved and because a reader of the coverage figure will not see it.**

**`GATE-025`, `GATE-026`, `GATE-027` and `GATE-028` are implemented, tested, and registered.
NOTHING IMPORTS THEM.** Measured: zero references to `gate_027_stop_ladder` anywhere under
`app/services/live/`, and `crypto_loop` still computes its stop from the pre-contract ICT path.

    coverage before   implemented 54 ids / 45 distinct   CANNOT FIRE 4   effective 47 / 40
    coverage after    implemented 58 ids / 49 distinct   CANNOT FIRE 5   effective 50 / 43

**`effective coverage` means ABLE TO REACH A VERDICT IF CALLED. It has never meant IS CALLED**,
and this task moves the figure up by three while changing what the live engine does by nothing.
**That is the correct behaviour for a shadow-only task and it is also how the number drifts away
from the thing anyone reading it cares about.**

**The wiring is a broker/engine-layer change and is deliberately out of scope** — criterion 10
says deploy is not required and nothing here is wired into `crypto_loop`. **Whoever wires it
meets `B115`'s comment problem too**, since the stop and the size are computed in the same place.

### B120 — a tie-break fixture built on a monotonic ladder cannot test the tie-break, and it passes identically either way

**Filed by Execute 2026-08-16, T-0025a. Measured, not reasoned — both cases were run against
both implementations.**

**`GATE-028` requires: among candidates clearing 2R, take the one closest to 3R, TIES TO THE
LARGER STOP.** The obvious fixture is the doctrine's own worked table with the 3R row dropped,
giving `|2−3| = |4−3| = 1`. **It is vacuous, and the reason is structural rather than careless.**

**`GATE-028` itself establishes that RR falls MONOTONICALLY as the stop widens.** So in a
cushion-monotonic ladder **the larger stop is ALWAYS EARLIER in ladder order**, and Python's
`min()` returns the FIRST minimum. **The correct answer and the list-order accident coincide by
construction.** Measured, by mutating the tie-break to a bare `min()` over `|rr − 3.0|`:

    CASE A  doctrine's own numbers, ladder order == cushion order    naive PASSES   correct PASSES
    CASE B  ladder order and cushion order DISAGREE                  naive FAILS    correct PASSES

**A test suite carrying only Case A is green against an implementation that resolves ties by list
position, which is not the rule.**

**Both cases are kept and they test different things:** Case A pins the DOCTRINE's direction,
which is where a reader's intuition fails — *ties to the larger stop* means **the LOWER RR**, the
2R candidate and not the 4R one. Case B pins the IMPLEMENTATION, and it is the only one that can
go red.

**Why Case B is legitimate rather than contrived:** it requires a NON-MONOTONIC ladder, and
**monotonicity is exactly the claim nobody has measured.** `GATE-027` asserts it in prose;
`T-0030`'s `1b-i` is the task that measures the inversion rate over a corpus. **A fixture that
assumed monotonicity in order to test the tie-break would be assuming the thing under test.**
...
### B123 — `B116`'s opposite direction: behaviour in the tree that NO rule id authorises, and it is invisible to every coverage number we publish

**Filed by the Manager 2026-08-16. `B116` counts rules that cannot fire. This counts code that fires
without a rule** — the same contract gap from the other side, **and the two are not symmetric in how
easily they are seen.**

    B116's direction   a rule exists, no producer supplies its input   -> reported CANNOT_FIRE_WITHOUT
    THIS direction     code determines behaviour, no rule id backs it  -> reported NOWHERE

**Two instances, both verified today rather than recalled:**

    live/crypto_loop.py:794   "instrument_class": "ALIGNED_MAJOR"    a hardcoded literal
    live/shadow.py:746        "instrument_class": "ALIGNED_MAJOR"    the same literal, second site
    ict/detector.py:192       def detect_order_blocks(...)           a COMPLETE order-block definition

**On the first:** `instrument_class` is exactly the field `B116` records as having **no producer** —
and the live path supplies it anyway, as a constant, in two places. **So the same field is
simultaneously "unproducible" in the rules layer and "always ALIGNED_MAJOR" in the live layer.** It is
also what makes `GATE-008` raise on an altcoin roster, so **a literal nobody ruled is load-bearing on
which symbols the platform can trade.**

**On the second:** `T-0029` established that **no rule in 117 defines an order block.** A working
detector for one has been in the tree the whole time. **Verified strictly today — no rule module
imports it** (`grep -rnE "^\s*(from|import)\s+.*\bict\b" rules/` returns nothing; the single
`services.ict` string in `rules/` is a comment at `gate_027_stop_ladder.py:44` explaining why it was
deliberately not reached for). **Correct today, and one import away from becoming doctrine by
accident.**

#### Why this is worth an entry rather than a shrug

**Every coverage figure this loop reports is a count of RULES** — implemented, distinct, effective,
cannot-fire. **Neither instance above is a rule, so neither can appear in any of them.** The figures
are honest about what they measure and **silent about a category they cannot see.**

> **`B116`'s gap announces itself** — a rule that cannot fire shows up as `NOT_EVALUABLE` with a
> `missing_producer`. **This gap has no id to be missing.** There is no slot for it to be absent from.

**Not a proposal to register these as rules.** Both are `[ENGINEERING]` facts, and inventing rule ids
for them is the exact move `T-0029` refuses. **What is missing is a place to COUNT them** — the
coverage report should state how many behaviours it does not govern, or stop implying that its
denominator is the system.

Related: **B115**, **B116**, **B44**. **`T-0030` criterion 3b files a third candidate** — `GATE-029` is
classified `HARD_GATE` and its output blocks nothing, which is the same mismatch inside the registry
...
**Filed by Execute 2026-08-16, after T-0024 passed. Review's observation, measured by Review and
reproduced independently by me before filing. The failed instrument is mine.**

**This register's recurring blocker is not rules without implementations. It is implementations
without PRODUCERS**, and four instances now share one structure:

    TARGET-001   active_institutional_destination   no producer   T-0023
    GRADE-019    instrument_class                   no producer   T-0024
    GATE-027     ORDER_BLOCK, ladder rung 4         no producer   found scoping T-0025 -> T-0029
    (B103)       main_asset_state                   no producer   in the schema, deliberately not required

### The exposure, with its denominator

    distinct implemented rule CLASSES        45     (= RULE_ID constants in source; 54 ids incl. aliases)
      declare CANNOT_FIRE_WITHOUT             4     GATE-040, GATE-041, GRADE-019, TARGET-001
      do NOT declare                         41

**`effective coverage` subtracts the cannot-fire bucket, and membership of that bucket is A
SELF-REPORT.** A rule lands in it only because its implementer noticed and declared
`CANNOT_FIRE_WITHOUT`. **So 41 rules are counted toward effective coverage on the strength of
nobody having said otherwise, and nothing verifies one of them.** The four that declared are the
four somebody happened to notice while building.

**THIS IS EXPOSURE, NOT A CLAIM THAT ANY OF THE 41 IS WRONG.** A real audit needs each rule's
`inputs` resolved to a producer, `inputs` is prose, and a crude scan would manufacture exactly the
artefact described below. **45 was confirmed from two directions** — distinct classes AND RULE_ID
constants in source — which is why the 41 is quotable rather than merely stated. (After `117 vs
104` and `103 vs 102`, adopting a denominator is how this file gets a wrong number in it.)

### THE CHEAP INSTRUMENT WAS TRIED AND IT FAILS. DO NOT REBUILD IT.

`depends_on` is a list of RULE IDS rather than prose, so it looks like the mechanically-auditable
half of the graph — `rule_waves.py`'s documented blind spot being the DATA-NAME half (B44).
Measured over the 45:

    classes with a rule-id depends_on        12     <- the instrument sees 27% of 45, no more
    classes with NO depends_on at all        33     <- prose inputs only, invisible to it
    undeclared dependency gaps found          1     PRIM-004 -> TARGET-005 (UNIMPLEMENTED)

**That single hit is a FALSE POSITIVE.** `PRIM-004`'s docstring opens *"THE RULE THIS PRIMITIVE
EXISTS TO ENFORCE IS TARGET-005"* and its `COVERAGE_NOTE` says *"TARGET-005 structural clearance
are built"* — it reads `TARGET-005.values.weak_sweep_penetration_pct` **as a registry constant** and
implements that rule's clearance itself. It fires perfectly well.

> **`depends_on` cannot distinguish "CONSUMES that rule's runtime output" from "READS that rule's
> registry constants, or enforces its doctrine". Those are opposite situations — one is a producer
> gap, the other is a rule doing exactly its job — and the graph renders them identically.**

**So: zero real findings across 27% coverage.** The rule-id graph cannot replace the 41 self-reports,
and a seat who tries will spend a cycle proving this again — the approach is obvious, cheap, and
looks correct right up to the point where you open the file it accuses.

**Caught by opening that file. The cheapest check available, and the one a confident-looking scan
actively discourages** — the fifth well-formed-but-wrong instrument result in a single day, after
two greps and two of Review's regexes. **None of the five errored.**

**What it could break:** the coverage figure is the programme's main progress signal and readiness
gate 5 is written against it. **An undeclared producer gap inflates `effective coverage` silently
and permanently**, and the rule sits green, registered, and deciding nothing — A10's *"an
implemented rule that decides nothing is furniture"* wearing a tick. **`T-0029` is the first task
in this programme that exists purely to close a producer gap, so it is the natural home for a real
instrument** — one that resolves declared inputs to producers. **Whether that is buildable at all is
the open question; `inputs` being prose is the obstacle.** Related: **B44**, **B54**, **B96**,
**B103**, **B115**.
...
### B83 — the ladder is asserted cushion-monotonic; the flag's reachability turns on the SELECTOR, not on monotonicity, and the tie-break is the half that is genuinely unobservable

> ## `[AMENDED 2026-08-16 — THE FIRST CONSEQUENCE BELOW IS WRONG. Heading corrected with it.]`
>
> **Found by the Manager scoping `T-0030`, re-derived and conceded by Review, who filed the original.**
>
> **The claim was:** *if the ladder is cushion-monotonic, `GATE-030`'s `TIGHTER_THAN_NECESSARY` can
> never fire.* **It does not hold under the selector that actually shipped.** `GATE-028`'s
> `operative_reading` is `CLOSEST_TO_3R_TIES_TO_LARGER`, with `rr_floor 2.0`, `rr_preferred 3.0`:
>
>     case                        widest  SHIPPED picks  flag?   B83's assumed selector  flag?
>     widest survivor >= 3R          3.1            3.1      -                      3.1      -
>     all survivors below 3R         2.1            2.9  FIRES                      2.1      -
>     survivors straddle 3R          2.2            3.1  FIRES                      2.2      -
>     single survivor                2.4            2.4      -                      2.4      -
>
> **Two of four cases fire on a perfectly monotonic ladder.** Under the selector `B83` assumed — *the
> first rung clearing 2R* — it never fires, which is what made the original derivation feel sound.
>
> > **So reachability is a function of the `GATE-028` READING, not of monotonicity.** Unreachable only
> > under a cushion-preferring selector; near-always-firing under `LARGEST_ABOVE_2R`; genuinely
> > reachable under what shipped.
>
> **THE SECOND CONSEQUENCE STANDS AND IS UNAFFECTED:** under monotonicity `GATE-028`'s **tie-break is
> unobservable** — `min()` returns the first minimum, the larger stop is always the earlier rung, so
> the correct answer and the list-order accident coincide by construction. **Verified numerically by
> Review and independently by Execute's Case A measurement inside `T-0025a`.**
>
> **Review's own account of the cause, which is the part worth keeping:** the consequence was derived
> from `GATE-025`'s *"evaluate the next tighter option"* walk and `GATE-027`'s monotonicity claim,
> **without opening `GATE-028` — the rule that OWNS selection** — which had already recorded
> `CLOSEST_TO_3R` as operative in v1.2.0, before `B83` was written. **Reasoning from a description of a
> rule instead of from the rule.** Same root as the `B81` citation chain and **B122**.
>
> **Consequence for `T-0030`:** its rationale is now **one** consequence, not two. *Monotonicity makes
> the tie-break unobservable* — singular. **Still a real reason to measure; a weaker one than filed.**
