...
export function ChartArea({ pair, timeframe, onTimeframeChange, score, aiBias }: ChartAreaProps) {
  ...
  const drawPositionLines = useCallback((raw: any[]) => {
  ...
  const drawTradeMarkers = useCallback((trades: any[]) => {
    const series = seriesRef.current
    if (!series) return
  ...
  useEffect(() => {
    fetch('/api/engine/status', { headers: authHeaders() }).then((r) => r.json()).then((d) => {
