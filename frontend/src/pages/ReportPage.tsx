      ...
      <div style={{ padding: '20px 28px', maxWidth: 1100 }}>
        ...
        <div style={{ background: 'rgba(167,139,250,0.06)', border: '1px solid #2a2545', borderRadius: 12, padding: '16px 18px' }}>
          ...
          <div style={{ fontSize: 13, color: '#b8b8c8', lineHeight: 1.6 }}>
            <strong style={{ color: '#ff3b5c' }}>These figures are not a usable track record and should not be relied on.</strong> A code review on
            2026-07-17 found that the backtest contains a <strong style={{ color: '#e8e8ef' }}>lookahead defect</strong>: the entry test reduces to
            <code style={{ color: '#e8e8ef' }}> low ≤ low</code>, so the simulator buys at each bar's exact low — a price no real order can obtain.
            Every figure above is inflated by that artifact, and the numbers will change materially once it is repaired.
            An earlier version of this page described these results as "lookahead-free" and the edge as "real but modest";
            <strong style={{ color: '#e8e8ef' }}> both statements were wrong</strong> and are retracted here.
            Separately, the strategy has <strong style={{ color: '#e8e8ef' }}>not</strong> passed out-of-sample robustness testing
            (Deflated Sharpe 0.255, against ~0.95 needed). Corrected figures will be published once the repair and re-run are complete.
          </div>
        </div>
      </div>
    </div>
